...
for triple in pythag_triples(max_length_side):
    ...
    if abs(2 * a - c) == 1:
        print(f"Pythag triple {triple} forms an almost equilateral triangle with perimeter {2*c + 2*a}, sides = {[c, c, 2*a]}")
        almost_equilateral.append([c, c, 2*a])
    if abs(2 * b - c) == 1:
        print(f"Pythag triple {triple} forms an almost equilateral triangle with perimeter {2*c + 2*b}, sides = {[c, c, 2*b]}")
