...
possible_happy_number_sums = np.zeros(
    (
        HAPPY_NUMBERS_MAX_DIGITS + 1,
        HAPPY_NUMBERS_BASE * HAPPY_NUMBERS_MAX_DIGITS,
        HAPPY_NUMBERS_BASE * HAPPY_NUMBERS_MAX_DIGITS,
    ),
    dtype=bool,
)
...
def listIncrementingHappyNumbers(
    num_buffer, last_digit, digits_remaining, sum_remaining
):
    if digits_remaining == 0:
        ...
        for digit in range(last_digit, HAPPY_NUMBERS_BASE):
            if possible_happy_number_sums[digits_remaining][sum_remaining][digit]:
                ...
                listIncrementingHappyNumbers(
                    num_buffer,
                    digit,
                    digits_remaining - 1,
                    sum_remaining - digit_powers[digits_remaining],
                )
