...
def get_fraction_parts(n):
    ...
    for i in range(1, n):
        numerator, denominator = denominator, (2 * denominator + numerator ) # Funky little swap - thanks python
        if len(str(numerator +denominator)) > len(str(denominator)): # Don't forget to add the demoninator to the numerator for the "+1" part
